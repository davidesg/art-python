"""
art.bugs — lightweight in-repo bug tracker.
...
new ones.  The workflow is:
    ...
    1.  file a report        →  ``art-bug new`` (or copy TEMPLATE.md)
...
This is a sibling of fue's tracker (fue.bugs); the two are kept deliberately
identical so the workflow, CLI, and file format transfer 1:1 between the two
projects.
"""
...
def project_name(bugs_dir=None) -> str:
    """El nombre del proyecto al que pertenece un registro de defectos.

    Estaba clavado —«**ART** (art-tseries)»— dentro de `render_index`, y en
    cuanto el registro sirvió a un segundo repositorio el índice de `pyfug`
    salió diciendo que era el de ART. Es la enfermedad de siempre: una
    propiedad del sitio, escrita en el código que la usa.

    Se toma, en este orden:
      1. `bugs/PROJECT` — una línea, si el repositorio quiere decirlo él;
      2. el nombre del directorio que contiene `bugs/`.
    """
    d = Path(bugs_dir) if bugs_dir else find_bugs_dir()
    if d is None:
        return "this repository"
    marca = Path(d) / "PROJECT"
    if marca.is_file():
        txt = marca.read_text(encoding="utf-8").strip().splitlines()
        if txt and txt[0].strip():
            return txt[0].strip()
    return Path(d).resolve().parent.name
...
def render_index(bugs_dir=None):
    ...
    lines = [
        "# Bug reports",
        "",
        f"In-repo bug tracker for **{project_name(bugs_dir)}**.  One Markdown "
        "file per bug (`BUG-NNNN-slug.md`); this index is generated by "
        "`art-bug index`.",
        "",
        "New report: `art-bug new` (or copy `TEMPLATE.md`).  "
        "Validate: `art-bug check`.  A fix commit references the id, e.g. "
        "`fix(pipeline): BUG-0001 …`.",
        "",
    ]
